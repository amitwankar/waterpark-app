...
export async function POST(req: NextRequest) {
  const { error } = await requireSubRole("COSTUME_ATTENDANT", "TICKET_COUNTER", "SALES_EXECUTIVE");
  ...
  try {
    ...
    const pending = await db.costumeRental.findMany({
      where: {
    ...
    if (pending.length === 0) {
      return NextResponse.json({ error: "No pending booked costumes found for this booking." }, { status: 409 });
    }
    ...
    return NextResponse.json({ error: "Failed to deliver booked costumes" }, { status: 500 });
  }
}
